...
const SingleInventory = () => {
    ...
    console.log('single service', service)
     ...
     const handleDelever = inventoryItem =>{
        const { name, price, imageURL, quantity } = inventoryItem;
         ...
         console.log(order)
         axios.post('https://warehouse-management-server-a11.onrender.com/deliveredNAME', order)
         .then(response => {
             const {data} = response;
             if(data.insertedId){
    ...
    const updateQuantity = e =>{
        ...
        if(e.target.quantity.value >0){
            ...
            fetch(`https://warehouse-management-server-a11.onrender.com/inventorie/${serviceId}`, {
                method: "PUT",
                headers: {
